...
export const executions = pgTable(
  ...
  (t) => [
    index('executions_workspace_id_created_at_idx').on(t.workspaceId, t.createdAt),
    index('executions_workflow_id_idx').on(t.workflowId),
    index('executions_pod_id_idx').on(t.podId),
  ],
);
...
export const executionLogs = pgTable(
  ...
  (t) => [index('execution_logs_execution_id_idx').on(t.executionId)],
);
>>>>>>> bfb8587 (LIN-53: Codebase cleanup - split oversized files, fix AI-slop patterns, audit fixes (#117))
