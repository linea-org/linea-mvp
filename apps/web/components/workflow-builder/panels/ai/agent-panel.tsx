...
const AGENT_TOOLS: Array<{
...
}> = [
  {
    name: "http_request",
    label: "HTTP Request",
    description: "Call any REST API or URL",
    approvalTag: "mutation",
  },
  {
    name: "run_javascript",
    label: "Run JavaScript",
    description: "Execute a JS snippet for data transforms",
    approvalTag: "always",
  },
  {
    name: "ask_human",
  ...
  },
  {
    name: "memory_store",
    label: "Memory: Store",
    description: "Persist facts across executions",
    approvalTag: "auto",
  },
  {
    name: "memory_search",
    label: "Memory: Search",
    description: "Recall facts from previous runs",
    approvalTag: "auto",
  },
  {
    name: "read_variable",
    label: "Read Variable",
    description: "Read a named workflow variable",
    approvalTag: "auto",
  },
  {
    name: "write_variable",
    label: "Write Variable",
    description: "Set a named workflow variable",
    approvalTag: "auto",
  },
]
  ...
  const base: SlashCmd[] = [
    {
      label: "{{lastOutput}}",
      insert: "{{lastOutput}}",
      desc: "Output from previous node",
    },
    {
      label: "{{variables.x}}",
      insert: "{{variables.}}",
      desc: "A named workflow variable",
    },
    {
      label: "{{input.x}}",
      insert: "{{input.}}",
      desc: "Workflow input field",
    },
    {
      label: "{{executionId}}",
      insert: "{{executionId}}",
      desc: "Current execution ID",
    },
  ...
  return (
    <div className="relative">
      ...
      {slashOpen && filtered.length > 0 && (
        <div className="absolute left-0 z-50 mt-0.5 w-full overflow-hidden rounded-md border border-border bg-popover shadow-lg">
          <p className="border-b border-border px-2.5 py-1 text-[9px] font-bold tracking-widest text-muted-foreground uppercase">
            Insert variable
          </p>
          ...
          <p className="border-t border-border px-2.5 py-1 text-[9px] text-muted-foreground">
            <kbd className="rounded border border-border px-1 py-0.5 text-[8px]">
              Enter
            </kbd>{" "}
            to insert &nbsp;|&nbsp;
            <kbd className="rounded border border-border px-1 py-0.5 text-[8px]">
      ...
      <p className="mt-1 text-[9px] text-muted-foreground">
        Type <code className="text-[9px]">/</code> to insert a variable
        reference. Drop a text file to embed its contents.
      </p>
    </div>
  )
}
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-1.5">
        ...
        <p className="text-[10px] text-muted-foreground">
          Sent as the system message. Defines the agent&apos;s persona and
          constraints.
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="space-y-1">
          <Label htmlFor="agent-maxsteps" className="text-[10px]">
            Max steps
          </Label>
          <Input
            id="agent-maxsteps"
      ...
      <div className="flex items-center justify-between">
        <Label htmlFor="agent-history" className="cursor-pointer">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <div>
            <Label className="cursor-pointer" htmlFor="agent-ltm">
              Long-term memory
            </Label>
            <p className="mt-0.5 text-[10px] text-muted-foreground">
              Inject memories from past executions into context
            </p>
          </div>
          <Switch
            id="agent-ltm"
      ...
      <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span
                      className={cn(
                        ...
                        APPROVAL_TAG_STYLES[tool.approvalTag]
                      )}
                    >
        ...
        {enabledTools.length === 0 && (
          <p className="text-[10px] text-muted-foreground/60 italic">
            No tools selected — the agent will respond without calling external
            services.
          </p>
        )}
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <div>
            <Label className="cursor-pointer" htmlFor="agent-structured">
              Structured output
            </Label>
            <p className="mt-0.5 text-[10px] text-muted-foreground">
        ...
        {!!data.outputSchema && (
          <div className="space-y-1.5">
            ...
            />
            <p className="text-[10px] text-muted-foreground">
              The model&apos;s output will be parsed as JSON matching this
              schema. The node output will be an object, not a string.
            </p>
          </div>
        )}
      </div>
