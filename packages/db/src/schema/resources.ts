...
export const resourceUsage = pgTable('resource_usage', {
  ...
  memoryMbSeconds: numeric('memory_mb_seconds', { precision: 12, scale: 3 }).notNull(),
  wallSeconds: numeric('wall_seconds', { precision: 10, scale: 3 }).notNull(),
  burstSeconds: numeric('burst_seconds', { precision: 10, scale: 3 }).default('0').notNull(),
  recordedAt: timestamp('recorded_at', { withTimezone: true }).defaultNow().notNull(),
});

export const resourcePoolsRelations = relations(resourcePools, ({ one }) => ({
  workspace: one(workspaces, { fields: [resourcePools.workspaceId], references: [workspaces.id] }),
}));
