...
export function CommandPalette({ open, onOpenChange }: CommandPaletteProps) {
  ...
  const navItems = [
    { label: 'Workflows', icon: WorkflowSquare01Icon, href: podBase ? `${podBase}/workflows` : null, shortcut: 'G W' },
    { label: 'Executions', icon: FlowCircleIcon, href: podBase ? `${podBase}/executions` : null, shortcut: 'G E' },
    { label: 'Schedules', icon: Calendar01Icon, href: podBase ? `${podBase}/schedules` : null, shortcut: 'G S' },
    { label: 'Webhooks', icon: LinkSquare01Icon, href: podBase ? `${podBase}/webhooks` : null },
    { label: 'Knowledge', icon: Database01Icon, href: '/knowledge', shortcut: 'G K' },
    { label: 'Templates', icon: GridViewIcon, href: '/templates', shortcut: 'G T' },
    { label: 'Metrics', icon: Analytics02Icon, href: '/metrics', shortcut: 'G M' },
  ...
  const filteredWorkflows = search
    ? recentWorkflows.filter((w) => w.name.toLowerCase().includes(search.toLowerCase()))
    : recentWorkflows;
          ...
          <CommandGroup heading="Navigate">
            {filteredNav.map(({ label, icon, href, shortcut }) => (
              ...
              >
                ...
                {label}
                {!href && <span className="ml-1 text-[10px] text-muted-foreground">(no active pod)</span>}
            ...
            <CommandGroup heading="Recent workflows">
              {filteredWorkflows.map((wf) => (
                <CommandItem
                  ...
                  onSelect={() => run(() => router.push(`/pods/${wf.podId}/workflows/${wf.id}`))}
                >
