        ...
        {vars.map((v) => {
          ...
          return (
            <div key={v.name} className="space-y-0.5">
              <label className="flex items-center gap-1 text-[9px] font-medium tracking-wide text-muted-foreground uppercase">
                {v.name}
              ...
              {v.type === "boolean" ? (
                ...
                <Textarea
                  ...
                  onChange={(e) => {
                    ...
                    try {
                      set(v.name, JSON.parse(raw))
                    } catch {
                      set(v.name, raw)
                    }
                  }}
                  className="min-h-[52px] resize-none font-mono text-[11px]"
                  placeholder="{}"
                />
              ) : (
                <Input
