  ...
  switch (op) {
    case 'now': {
      ...
      return {
        ...
        formatted: formatDate(d, fmt),
      };
    }

    case 'format': {
      const raw = nodeData.input
        ? (state.variables[nodeData.input as string] ??
    ...
    case 'subtract': {
      ...
      return {
        ...
        formatted: formatDate(result, fmt),
      };
    }

    case 'diff': {
      const rawA = nodeData.dateA
        ? (state.variables[nodeData.dateA as string] ??
          state.variables.lastOutput)
        : state.variables.lastOutput;
    ...
    default:
      throw new Error(`Unknown datetime operation: ${op as string}`);
  }
}
