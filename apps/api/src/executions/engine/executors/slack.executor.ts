...
export async function executeSlackNode(
  ...
  if (!token)
    throw new Error(
      'Slack token not found. Connect Slack in Settings → Connections or add a SLACK_TOKEN secret.',
    );
  ...
  switch (action) {
    ...
    case 'send_dm': {
      if (!nodeData.userId)
        throw new Error('Slack: userId is required for DMs');
      if (!nodeData.message) throw new Error('Slack: message is required');
      ...
      const openJson = (await openResp.json()) as {
        ok: boolean;
        channel?: { id: string };
        error?: string;
      };
      ...
      const dmJson = (await dmResp.json()) as {
        ok: boolean;
        ts?: string;
        error?: string;
      };
      if (!dmJson.ok)
        throw new Error(`Slack DM send failed: ${dmJson.error ?? 'unknown'}`);
    ...
    case 'list_channels': {
      ...
      if (!json.ok)
        throw new Error(
          `Slack list_channels failed: ${json.error ?? 'unknown'}`,
        );
      return {
    ...
    default:
      throw new Error(`Slack: unknown action "${String(action)}"`);
  }
}
