...
The LangGraph engine calls `interrupt()`, which suspends the graph. The execution `status` changes to `suspended` in the database.
</Step>
<Step>
...
<Callout type="info">
  Suspended executions timeout after **10 minutes** of SSE inactivity (the stream disconnects). The execution record remains in `suspended` state in the database indefinitely until a response is submitted. There is no automatic timeout on the suspension itself.
</Callout>
