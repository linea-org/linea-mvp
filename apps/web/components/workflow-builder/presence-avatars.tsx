...
export function PresenceAvatars({ token, workspaceId, podId, workflowId }: Props) {
  ...
  return (
    <div className="flex items-center" title={others.map((u) => u.name ?? u.email).join(', ')}>
      {visible.map((u, i) => (
        <div
          key={u.userId}
      ...
      {overflow > 0 && (
        <div
          className="relative flex size-6 items-center justify-center rounded-full bg-muted ring-2 ring-background text-[10px] font-medium text-muted-foreground"
          style={{ marginLeft: -8 }}
        >
          +{overflow}
        </div>
