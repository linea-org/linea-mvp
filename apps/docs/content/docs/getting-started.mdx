...
This runs `drizzle-kit push` against your PostgreSQL instance and creates all tables.
...
| **Docs** (Fumadocs) | `http://localhost:3002` |
...
3. Select your workflow and provide input JSON:
   ```json
   { "message": "Explain how LangGraph works in 3 sentences." }
   ```
4. Click **Run**.
...
curl -X POST http://localhost:3001/v1/workspaces/<wsId>/pods/<podId>/executions \
  ...
  -d '{
    "workflowId": "<wfId>",
    "input": { "message": "Explain how LangGraph works in 3 sentences." }
  }'
```
