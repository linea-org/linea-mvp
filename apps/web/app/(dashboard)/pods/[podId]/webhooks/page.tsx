...
>>>>>>> bfb8587 (LIN-53: Codebase cleanup - split oversized files, fix AI-slop patterns, audit fixes (#117))
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      ...
      const api = createApiClient(token)
      return api.delete(
        `/workspaces/${activeWorkspace!.id}/pods/${podId}/webhooks/${id}`
      )
    },
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: webhooksKey })
...
>>>>>>> bfb8587 (LIN-53: Codebase cleanup - split oversized files, fix AI-slop patterns, audit fixes (#117))
    ...
    if (search.trim()) {
      ...
      list = list.filter((wh) =>
        workflowName(wh.workflowId).toLowerCase().includes(q)
      )
    }
    return list
      ...
      {isLoading || wsLoading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed py-16 text-center">
          ...
          <p className="mt-1 max-w-xs text-xs text-muted-foreground">
            Webhooks let external services trigger a workflow via a signed HTTP
            request.
          </p>
          <Button
            size="sm"
                  ...
                  <TableCell>
                    <div className="flex items-center justify-end gap-1">
                      ...
                      >
                        <HugeiconsIcon
                          icon={
                            rotatingId === wh.id ? Loading01Icon : RefreshIcon
                          }
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {revealSecret &&
              webhooks.some(
                (w) => w.id === revealSecret.webhookId && w.createdAt
              )
          ...
          <div className="space-y-3 py-2">
            <div className="space-y-1.5">
              ...
              <div className="flex items-center gap-2">
                ...
                />
                {revealSecret && (
                  <CopyButton value={triggerUrl(revealSecret.webhookId)} />
                )}
              </div>
            </div>
            <div className="space-y-1.5">
...
>>>>>>> bfb8587 (LIN-53: Codebase cleanup - split oversized files, fix AI-slop patterns, audit fixes (#117))
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. Any service using this webhook will
