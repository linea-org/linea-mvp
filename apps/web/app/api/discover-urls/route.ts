...
async function fetchText(url: string): Promise<string | null> {
  try {
    ...
    if (ct.includes('octet-stream') || ct.includes('image') || ct.includes('video')) return null;
...
async function parseSitemap(url: string, depth = 0): Promise<string[]> {
  ...
  if (text.includes('<sitemapindex') || locs.some((l) => l.endsWith('.xml') || l.includes('sitemap'))) {
    const results = await Promise.all(locs.slice(0, 10).map((l) => parseSitemap(l, depth + 1)));
    return results.flat();
  }
...
async function scrapeLinks(pageUrl: string, host: string, pathPrefix: string): Promise<string[]> {
  ...
  while ((match = hrefRe.exec(text)) !== null) {
    ...
    try {
      ...
      if (abs.host === host && isUnderPath(abs.href, host, pathPrefix)) {
        ...
        const normalized = abs.pathname !== '/' ? abs.href.replace(/\/$/, '') : abs.href;
 ...
 */
function isUnderPath(urlStr: string, host: string, pathPrefix: string): boolean {
  try {
...
export async function POST(req: NextRequest) {
  ...
  if (!urlStr) return NextResponse.json({ error: 'url is required' }, { status: 400 });
  ...
  const sitemapCandidates = sitemapUrlsFromRobots.length > 0
    ...
    : [`${origin}/sitemap.xml`, `${origin}/sitemap_index.xml`, `${origin}/sitemap.xml.gz`];
  ...
  for (const sitemapUrl of sitemapCandidates.slice(0, 3)) {
    ...
    for (const u of urls) {
      ...
      if (hasPathFilter ? isUnderPath(u, host, pathPrefix) : isSameHost(u, host)) {
        discovered.add(u);
      }
