...
const PLAN_LIMITS: Record<
  string,
  { executions: string; workflows: string; members: string }
> = {
...
>>>>>>> bfb8587 (LIN-53: Codebase cleanup - split oversized files, fix AI-slop patterns, audit fixes (#117))
  ...
  const paidPlans: PlanInfo[] = isLoading
    ? []
    : (plansData?.plans ?? FALLBACK_PLANS).filter(
  ...
  async function handleUpgrade(planKey: string) {
    ...
    try {
      ...
      const { url } = await api.post<{ url: string }>(
        `/workspaces/${wsId}/billing/checkout`,
        { plan: planKey }
      )
      ...
      {successPlan && (
        <div className="flex items-center gap-3 rounded-lg border border-green-500/30 bg-green-500/10 px-4 py-3">
          ...
          <div>
            ...
            <p className="mt-0.5 text-xs text-green-500/80">
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              {paidPlans.map((p) => {
                ...
                const isDowngrade =
                  (PLAN_RANK[currentPlan] ?? 0) > (PLAN_RANK[p.key] ?? 0)
