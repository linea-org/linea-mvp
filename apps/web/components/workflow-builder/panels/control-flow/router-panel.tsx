    ...
    <div className="space-y-4">
      ...
      <div className="rounded-md border border-border bg-muted/30 px-2.5 py-2">
        <p className="text-[11px] leading-snug text-muted-foreground">
          ...
          <span className="font-semibold text-foreground">top to bottom</span>.
          The first matching condition fires. If no route matches, execution
          stops.
        </p>
      </div>
        ...
        {routes.length === 0 ? (
          <div className="rounded-md border border-dashed border-border py-6 text-center">
            ...
            <p className="mt-0.5 text-[11px] text-muted-foreground/60">
              Add a route to define branching paths.
            </p>
          </div>
        ) : (
          <div className="space-y-2">
              ...
              >
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="flex-1 text-[10px] font-semibold tracking-wider text-muted-foreground uppercase">
                    Route {i + 1}
                    {route.id && (
                      <span className="ml-1.5 font-mono tracking-normal text-muted-foreground/60 normal-case">
                        · handle: {route.id}
                      </span>
                    )}
                  </span>
        ...
        <div className="space-y-2">
          <div className="rounded-md border border-amber-200 bg-amber-50 px-2.5 py-2 dark:border-amber-900 dark:bg-amber-950/30">
            <p className="text-[11px] leading-snug text-amber-700 dark:text-amber-400">
              Add a route with condition <span className="font-mono">true</span>{" "}
              as the last route to act as a catch-all fallback.
            </p>
          </div>
          {routes.length >= MAX_ROUTES && (
            <p className="text-[11px] text-muted-foreground">
              Maximum {MAX_ROUTES} routes reached. Chain another Router node for
