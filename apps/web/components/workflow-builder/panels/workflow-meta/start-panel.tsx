...
const TRIGGERS: {
...
}[] = [
  {
    ...
    label: "Manual",
    icon: PlayIcon,
    description: "Run from the builder or API",
  },
  {
    value: "webhook",
    label: "Webhook",
...
function describeCron(expr: string): string {
  ...
  if (month !== "*") {
    const months = month
      .split(",")
      .map((m) => MONTH_NAMES[parseInt(m) - 1] ?? m)
      .join(", ")
            ...
            <div className="grid grid-cols-2 gap-1">
              {CRON_PRESETS.map((p) => (
                <button
                  ...
                  className={cn(
                    "rounded-md border px-2 py-1.5 text-left text-[10px] font-medium transition-colors",
          ...
          <div className="space-y-1.5">
            ...
            <div className="grid grid-cols-5 gap-1 text-center">
              {["min", "hour", "day", "mon", "wday"].map((f) => (
                <span
                  key={f}
                  className="text-[9px] font-bold tracking-wider text-muted-foreground/60 uppercase"
                >
                  {f}
                </span>
              ))}
...
>>>>>>> bfb8587 (LIN-53: Codebase cleanup - split oversized files, fix AI-slop patterns, audit fixes (#117)):apps/web/components/workflow-builder/panels/workflow-meta/start-panel.tsx
          ...
          <p className="text-[10px] text-muted-foreground">
            ...
            <span className="font-mono">⌘ toolbar → Webhook</span>) to get your
            endpoint URL. Incoming POST body is available as input variables.
          </p>
        </div>
      )}
          ...
          {vars.length === 0 ? (
            ...
            <div className="space-y-2">
              ...
              {vars.map((v, i) => (
                <div
                  key={i}
                  className="grid grid-cols-[1fr_80px_32px_24px] items-center gap-1"
                >
                  <Input
                    value={v.name}
                  ...
                  <Button
                    ...
                    onClick={() =>
                      onUpdate({
                        inputVariables: vars.filter((_, j) => j !== i),
                      })
                    }
                  >
...
>>>>>>> bfb8587 (LIN-53: Codebase cleanup - split oversized files, fix AI-slop patterns, audit fixes (#117)):apps/web/components/workflow-builder/panels/workflow-meta/start-panel.tsx
          ...
          <p className="text-[10px] text-muted-foreground">
            Used when the public API receives a natural-language message and
            needs to extract typed input variables from it.
          </p>
        </div>
      )}
