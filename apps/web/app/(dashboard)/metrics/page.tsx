  ...
  return (
    <div className="flex flex-col items-center justify-center py-24 text-center">
      <div className="flex size-14 items-center justify-center rounded-2xl bg-muted mb-4">
        <HugeiconsIcon icon={AnalyticsUpIcon} className="size-7 text-muted-foreground" />
      </div>
      <h2 className="text-base font-semibold">No executions yet</h2>
      <p className="mt-1 text-sm text-muted-foreground max-w-xs">
        Run a workflow to start seeing execution stats, success rates, and duration percentiles here.
      </p>
      {firstPodId ? (
    ...
    <div className="space-y-6">
      ...
      {loading || wsLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          <p className="text-sm text-muted-foreground">Failed to load metrics. Try refreshing.</p>
        </div>
      ) : !hasData ? (
        <EmptyMetrics />
      ) : (
        <>
          ...
          {data.tokens && data.tokens.totalTokens > 0 && (
            <div className="grid grid-cols-3 gap-4">
              <StatCard
                label="Input tokens"
                value={formatTokenCount(data.tokens.totalInputTokens)}
                sub="sent to model"
              />
              <StatCard
                label="Output tokens"
                value={formatTokenCount(data.tokens.totalOutputTokens)}
                sub="generated by model"
              />
              <StatCard
                label="Total tokens"
                value={formatTokenCount(data.tokens.totalTokens)}
          ...
          {data.topWorkflows.length === 0 && (
            <div className="flex items-center gap-3 rounded-lg border border-dashed p-4 text-sm text-muted-foreground">
              <HugeiconsIcon icon={WorkflowSquare01Icon} className="size-4 shrink-0" />
