...
function getNodeProperties(
  ...
  switch (nodeType) {
    ...
    case "http":
      ...
      rows.push(
        data.authType ? { key: "auth", value: fmt(data.authType) } : null
      )
      break
    case "if-else":
      rows.push(
        data.condition ? { key: "if", value: fmt(data.condition, 22) } : null
      )
      break
    case "router": {
      const r = (data.routes as Array<{ label: string }> | undefined) ?? []
      rows.push(
        r.length > 0 ? { key: "routes", value: String(r.length) } : null
      )
      break
    }
    case "code":
      rows.push(
        data.language ? { key: "lang", value: fmt(data.language) } : null
      )
      break
    case "memory": {
      rows.push(
        data.memoryMode ? { key: "mode", value: fmt(data.memoryMode) } : null
      )
      rows.push(
        data.memoryScope ? { key: "scope", value: fmt(data.memoryScope) } : null
      )
      break
    }
    case "mcp":
      rows.push(
        data.serverName ? { key: "server", value: fmt(data.serverName) } : null
      )
      rows.push(
        data.toolName ? { key: "tool", value: fmt(data.toolName) } : null
      )
      break
    case "slack":
      rows.push(
        data.channel ? { key: "channel", value: fmt(data.channel) } : null
      )
    ...
    case "gmail":
      ...
      rows.push(
        data.subject ? { key: "subject", value: fmt(data.subject) } : null
      )
      break
    case "github":
    ...
    case "wait":
      rows.push(
        data.duration ? { key: "wait", value: `${fmt(data.duration)}s` } : null
      )
      break
    case "loop":
      rows.push(
        data.maxIterations
          ? { key: "max", value: String(data.maxIterations) }
          : null
      )
      rows.push(
        data.arrayPath ? { key: "over", value: fmt(data.arrayPath, 20) } : null
      )
      break
    case "evaluator":
      rows.push(data.model ? { key: "model", value: fmt(data.model) } : null)
    ...
    case "retriever": {
      rows.push(
        data.knowledgeBaseId
          ? { key: "kb", value: fmt(data.knowledgeBaseId, 12) }
          : null
      )
      rows.push(
        data.embeddingModel
          ? { key: "embed", value: fmt(data.embeddingModel, 20) }
          : null
      )
      break
    }
    case "extract":
      rows.push(data.prompt ? { key: "prompt", value: fmt(data.prompt) } : null)
      break
    case "approval":
    case "approval-gate":
      rows.push(
        data.message || data.approvalMessage
          ? {
              key: "prompt",
              value: fmt((data.message || data.approvalMessage) as string),
            }
          : null
      )
      break
    case "guardrails": {
      ...
      rows.push(
        rl.length > 0 ? { key: "rules", value: String(rl.length) } : null
      )
      break
    }
    case "subworkflow":
      rows.push(
        data.workflowId
          ? { key: "workflow", value: fmt(data.workflowId, 20) }
          : null
      )
      break
    case "parallel": {
      ...
      rows.push(
        b.length > 0 ? { key: "branches", value: String(b.length) } : null
      )
      break
    }
    case "start":
      rows.push(
        data.triggerType
          ? { key: "trigger", value: fmt(data.triggerType) }
          : null
      )
      break
    case "transform":
      rows.push(
        data.expression
          ? { key: "expr", value: fmt(data.expression, 22) }
          : null
      )
      break
    case "variables": {
    ...
    case "filter":
      ...
      rows.push(
        data.condition ? { key: "where", value: fmt(data.condition) } : null
      )
      break
    case "merge": {
      const srcs = (data.sources as string[] | undefined) ?? []
      rows.push(
        srcs.length > 0 ? { key: "sources", value: String(srcs.length) } : null
      )
    ...
    case "datetime":
      rows.push(
        data.operation ? { key: "op", value: fmt(data.operation) } : null
      )
...
const BRANCH_DEFS: Record<string, [BranchSide, BranchSide]> = {
  "if-else": [
    { id: "true", label: "T", cls: TRUE_CLS },
    { id: "false", label: "F", cls: FALSE_CLS },
  ],
  approval: [
    { id: "approved", label: "approved", cls: TRUE_CLS },
    { id: "rejected", label: "rejected", cls: FALSE_CLS },
  ],
  "approval-gate": [
    { id: "approved", label: "approved", cls: TRUE_CLS },
    { id: "rejected", label: "rejected", cls: FALSE_CLS },
  ...
  evaluator: [
    { id: "passed", label: "passed", cls: TRUE_CLS },
    { id: "failed", label: "failed", cls: FALSE_CLS },
  ],
  guardrails: [
    { id: "pass", label: "pass", cls: TRUE_CLS },
    { id: "block", label: "block", cls: FALSE_CLS },
  ],
}
...
export const CustomNode = memo(function CustomNode({
  ...
  const branchTrue = branchDef
    ? nodeType === "if-else"
      ? {
          ...branchDef[0],
          label: (data.trueLabel as string | undefined) || branchDef[0].label,
        }
      : branchDef[0]
    : null
  const branchFalse = branchDef
    ? nodeType === "if-else"
      ? {
          ...branchDef[1],
          label: (data.falseLabel as string | undefined) || branchDef[1].label,
        }
      : branchDef[1]
    : null

  const routes = isRouter
    ? ((data.routes as Array<{ id?: string; label: string }>) ?? []).map(
        (r, i) => ({ ...r, id: r.id ?? `route-${i}` })
      )
    : []
          ...
          {portsVertical ? (
            <>
              <Handle
                ...
                id={branchTrue.id}
                style={{ left: "30%" }}
                className={branchTrue.cls}
              />
              <span
                className="pointer-events-none absolute bottom-[-16px] max-w-12 truncate text-[9px] font-bold tracking-wide text-green-600 uppercase select-none"
              ...
              <Handle
                ...
                id={branchFalse.id}
                style={{ left: "70%" }}
                className={branchFalse.cls}
              />
              <span
                className="pointer-events-none absolute bottom-[-16px] max-w-12 truncate text-[9px] font-bold tracking-wide text-red-500 uppercase select-none"
                style={{ left: "calc(70% - 14px)" }}
              >
                {branchFalse.label}
              </span>
            </>
            ...
            <>
              ...
              />
              <span
                className="pointer-events-none absolute right-[-4px] max-w-16 translate-x-full truncate text-[9px] font-bold tracking-wide text-green-600 uppercase select-none"
              ...
              />
              <span
                className="pointer-events-none absolute right-[-4px] max-w-16 translate-x-full truncate text-[9px] font-bold tracking-wide text-red-500 uppercase select-none"
                style={{ top: "calc(65% - 6px)" }}
              >
                {branchFalse.label}
              </span>
            </>
        ...
        routes.map((route, i) => {
            ...
            return (
              <React.Fragment key={route.id}>
                <Handle
                  ...
                  id={route.id}
                  style={{ left: `${pct}%` }}
                  className={SOURCE_CLS}
                />
                <span
                  className="pointer-events-none absolute bottom-[-16px] max-w-12 truncate text-[9px] font-medium text-muted-foreground select-none"
          ...
          return (
            <React.Fragment key={route.id}>
              ...
              />
              <span
                className="pointer-events-none absolute right-[-4px] max-w-16 translate-x-full truncate text-[9px] font-medium text-muted-foreground select-none"
...
export const FrameNode = memo(function FrameNode({
  ...
  const [labelValue, setLabelValue] = useState(
    (data.frameName as string) ?? "Group"
  )
  ...
  function toggleCollapse() {
    ...
    setNodes((nds) => {
      ...
      return nds.map((n) => {
        if (n.id === id) {
          const currentH =
            typeof n.style?.height === "number" ? n.style.height : 220
          return {
            ...n,
            style: {
              ...(n.style ?? {}),
              height: nowCollapsed
                ? 40
                : ((n.data.expandedHeight as number) ?? 220),
            },
            data: {
              ...n.data,
  ...
  function commitLabel() {
    ...
    setNodes((nds) =>
      nds.map((n) =>
        n.id === id ? { ...n, data: { ...n.data, frameName: labelValue } } : n
      )
    )
  }
      ...
      nds.map((n) =>
        n.id === id
          ? {
              ...
              data: { ...n.data, expandedHeight: height },
            }
          : n
      )
    )
  ...
  return (
    <div
      className={cn("overflow-hidden rounded-xl border-2 select-none")}
      style={{
      ...
      <div className="flex items-center gap-1.5 px-2.5 py-2">
        ...
        {editingLabel ? (
          <input
            ...
            className="nodrag min-w-0 flex-1 border-b bg-transparent text-xs font-semibold outline-none"
    ...
    >
      <div className="mb-1.5 flex items-center gap-1.5">
        <HugeiconsIcon
          ...
          className="size-3 text-yellow-700"
          strokeWidth={1.5}
        />
        <span className="text-[9px] font-bold tracking-wider text-yellow-700 uppercase">
          Note
        </span>
      </div>
      {editing ? (
        <textarea
          ...
          className="min-h-10 w-full cursor-text resize-none border-none bg-transparent text-xs text-yellow-900 outline-none"
        />
      ) : (
        <p className="m-0 min-h-10 text-xs whitespace-pre-wrap text-yellow-900">
          {text || (
