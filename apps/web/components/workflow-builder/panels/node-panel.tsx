...
const NODE_DOCS: Record<string, NodeDoc> = {
  start: {
    summary:
      "Entry point of the workflow. Every execution begins here. Supports manual, scheduled, webhook, and event triggers.",
    fields: [
      {
        name: "Trigger type",
        desc: "How the workflow is initiated: Manual, Schedule (cron), Webhook (HTTP POST), or Event.",
      },
      {
        name: "Cron expression",
        desc: "Standard cron syntax for scheduled runs (e.g. 0 9 * * 1 = every Monday at 9 AM).",
      },
      {
        name: "Webhook secret",
        desc: "Optional HMAC secret to verify incoming webhook payloads.",
      },
    ],
    ...
    summary:
      "Marks successful workflow termination. Any data passed into the End node becomes the workflow's final output.",
    inputs: ["result — the final output value of the workflow"],
    tips: ["You can have multiple End nodes to support branching exits."],
  },
  agent: {
    summary:
      "Runs an LLM agent with a system prompt, tool access, and optional memory. The agent reasons iteratively until it produces a final answer.",
    fields: [
      {
        name: "Model",
        desc: "Which LLM to use (e.g. Claude Sonnet, GPT-4o, Gemini Pro).",
      },
      {
        name: "System prompt",
        desc: "Persistent instructions that shape agent behavior and persona.",
      },
      {
        name: "User message",
        desc: "The task or question sent to the agent each run. Supports {{variable}} interpolation.",
      },
      {
        name: "Max iterations",
        desc: "Safety cap on the reasoning loop to prevent runaway costs.",
      },
      {
        name: "Tools",
        desc: "List of tools the agent can invoke (e.g. search, HTTP, MCP).",
      },
      { name: "Memory", desc: "Enable to persist context across executions." },
    ],
    inputs: [
      "context — data passed from upstream nodes, available as variables",
    ],
  ...
  http: {
    summary:
      "Makes an outbound HTTP request to any REST API. Handles auth, headers, query params, and JSON/form bodies.",
    fields: [
      { name: "Method", desc: "GET, POST, PUT, PATCH, DELETE." },
      {
        name: "URL",
        desc: "Target endpoint. Supports {{variable}} interpolation.",
      },
      { name: "Headers", desc: "Key-value pairs sent with every request." },
      { name: "Auth", desc: "Bearer token, Basic, or API key authentication." },
      {
        name: "Body",
        desc: "JSON or form-encoded payload for POST/PUT/PATCH.",
      },
      {
        name: "Timeout",
        desc: "Max wait time in milliseconds before failing.",
      },
    ],
  ...
  transform: {
    summary:
      "Reshapes data using JSONata expressions or simple field mappings. Use it to extract, rename, or restructure fields between nodes.",
    fields: [
      {
        name: "Expression",
        desc: "JSONata query or JS-like expression applied to input data.",
      },
      {
        name: "Output key",
        desc: "The key under which the result is stored in the workflow context.",
      },
    ],
  ...
  "if-else": {
    summary:
      "Branches workflow execution based on a boolean condition. Routes to True or False paths independently.",
    fields: [
      {
        name: "Condition",
        desc: 'A JavaScript expression that evaluates to true or false (e.g. data.status === "active").',
      },
    ],
    inputs: ["data — the context object evaluated by the condition"],
    outputs: [
      "true — taken when condition is truthy",
      "false — taken when condition is falsy",
    ],
    tips: [
  ...
  router: {
    summary:
      "Routes to one of N named output branches based on matching a value against defined cases. Equivalent to a switch/match statement.",
    fields: [
      {
        name: "Value expression",
        desc: "Expression whose result is matched against branch labels.",
      },
      {
        name: "Branches",
        desc: 'Named output handles, each with a matching value or "default".',
      },
    ],
    inputs: ["data — context object from which the value is extracted"],
    outputs: [
      "[branch name] — one output per defined route; only the matched branch fires",
    ],
    tips: [
  ...
  approval: {
    summary:
      "Pauses the workflow and sends an approval request (email, Slack, etc.). Execution resumes only after a human approves or rejects.",
    fields: [
      {
        name: "Approvers",
        desc: "Comma-separated email addresses or user IDs.",
      },
      { name: "Message", desc: "Explanation shown to the approver." },
      {
        name: "Timeout",
        desc: "Duration before auto-rejecting if no response is received.",
      },
      {
        name: "Channel",
        desc: "How to notify approvers: Email, Slack, or in-app.",
      },
    ],
    inputs: ["data — payload shown in the approval request"],
    outputs: [
      "approved — path taken on approval",
      "rejected — path taken on rejection",
    ],
    tips: [
  ...
  mcp: {
    summary:
      "Calls a tool exposed via the Model Context Protocol (MCP). Connects to MCP servers to access external tools and data sources.",
    fields: [
      { name: "Server", desc: "MCP server URL or configured server name." },
      {
        name: "Tool",
        desc: "The specific tool name to invoke on that server.",
      },
      {
        name: "Arguments",
        desc: "JSON arguments passed to the tool, supports {{variable}} interpolation.",
      },
    ],
  ...
  memory: {
    summary:
      "Reads from or writes to persistent workflow memory. Use it to maintain state across runs, store user preferences, or accumulate history.",
    fields: [
      {
        name: "Operation",
        desc: "Read — retrieve stored values; Write — save new values.",
      },
      {
        name: "Scope",
        desc: "Pod-level (shared), Workflow-level, or User-level memory.",
      },
      { name: "Key", desc: "Identifier for the stored value." },
      {
        name: "Value",
        desc: "Data to write (Write operation only). Supports {{variable}} interpolation.",
      },
      {
        name: "TTL",
        desc: "Optional time-to-live in seconds before the value expires.",
      },
    ],
  ...
  extract: {
    summary:
      "Extracts structured data from unstructured text using an LLM. Specify a JSON schema and the node returns typed, validated fields.",
    fields: [
      {
        name: "Input text",
        desc: "Source text to extract from. Supports {{variable}} interpolation.",
      },
      {
        name: "Schema",
        desc: "JSON Schema defining fields to extract (name, type, description).",
      },
      { name: "Model", desc: "LLM used for extraction." },
      {
        name: "Strict mode",
        desc: "Fail if any required schema field is missing from the output.",
      },
    ],
    inputs: [
      "text — unstructured content (e.g. email body, document, API response)",
    ],
  ...
  retriever: {
    summary:
      "Performs semantic search over a knowledge base and returns relevant document chunks. Used to ground agents in factual content.",
    fields: [
      {
        name: "Knowledge base",
        desc: "Which indexed knowledge source to query.",
      },
      {
        name: "Query",
        desc: "Search query — usually the user's question or a derived search term.",
      },
      { name: "Top K", desc: "Number of document chunks to return." },
      {
        name: "Similarity threshold",
        desc: "Minimum relevance score to include a result (0–1).",
      },
    ],
    inputs: ["query — text to search with"],
    outputs: [
      "chunks — array of matching document excerpts with metadata and scores",
    ],
    tips: [
  ...
  guardrails: {
    summary:
      "Validates content against safety, compliance, or custom policies. Blocks or flags content that violates defined rules before it reaches downstream nodes.",
    fields: [
      { name: "Input", desc: "The text or data to validate." },
      {
        name: "Policies",
        desc: "List of active guardrail policies (e.g. no PII, no harmful content).",
      },
      {
        name: "Action on violation",
        desc: "Block (halt execution), Redact (remove violating content), or Flag (annotate and continue).",
      },
    ],
    inputs: ["content — text or structured data to evaluate"],
    outputs: [
      "safe — content that passed all checks",
      'violation — triggered when action is "Flag"',
    ],
    tips: [
  ...
  code: {
    summary:
      "Executes a custom JavaScript or Python snippet in a sandboxed environment. Use it for transformations, calculations, or logic that can't be expressed in other nodes.",
    fields: [
      { name: "Language", desc: "JavaScript or Python." },
      {
        name: "Code",
        desc: "The script body. Receives `input` object; must return a value.",
      },
  ...
  loop: {
    summary:
      "Iterates over an array and runs the connected subgraph once per item. Supports parallel or sequential execution.",
    fields: [
      {
        name: "Iterator",
        desc: "Expression resolving to the array to loop over.",
      },
      {
        name: "Item variable",
        desc: "Name used to reference the current item inside the loop body.",
      },
      {
        name: "Mode",
        desc: "Sequential (one item at a time) or Parallel (all items concurrently).",
      },
      {
        name: "Max concurrency",
        desc: "Parallel mode only — max simultaneous executions.",
      },
    ],
    inputs: ["items — the array to iterate"],
    outputs: [
      "results — array of outputs from each iteration",
      "item — the current iteration's output inside the loop",
    ],
    tips: [
  ...
  parallel: {
    summary:
      "Forks execution into multiple simultaneous branches and waits for all to complete before continuing.",
    fields: [
      {
        name: "Branches",
        desc: "Named output handles — one per parallel path.",
      },
      {
        name: "Wait for all",
        desc: "When enabled, halts until every branch finishes (default). Disable to use first-completed mode.",
      },
    ],
    inputs: ["data — shared context forwarded to every branch"],
    outputs: [
      "[branch name] — each branch receives the full input context",
      "merged — downstream receives all branch outputs combined",
    ],
    tips: [
  ...
  wait: {
    summary:
      "Pauses workflow execution for a fixed duration or until a specific datetime.",
    fields: [
      {
        name: "Mode",
        desc: "Duration (wait N seconds/minutes/hours) or Until (wait until a specific ISO timestamp).",
      },
      {
        name: "Duration",
        desc: "Wait time in the chosen unit (Duration mode).",
      },
      {
        name: "Until",
        desc: "ISO 8601 datetime to resume at (Until mode). Supports {{variable}} interpolation.",
      },
    ],
  ...
  variables: {
    summary:
      "Declares and sets workflow-scoped variables. Use it to initialize or update named values accessible by downstream nodes.",
    fields: [
      {
        name: "Variables",
        desc: "Name-value pairs to set. Values support {{variable}} interpolation.",
      },
      {
        name: "Merge mode",
        desc: "Replace (overwrite existing) or Merge (deep-merge with existing context).",
      },
    ],
  ...
  evaluator: {
    summary:
      "Scores or evaluates an agent output using LLM-as-a-judge or rule-based criteria. Useful for quality gates and automated testing.",
    fields: [
      {
        name: "Input",
        desc: "The content to evaluate (usually an agent output).",
      },
      {
        name: "Criteria",
        desc: "Natural language description of what a good output looks like.",
      },
    ...
    outputs: [
      "score — numeric rating or pass/fail result",
  ...
  subworkflow: {
    summary:
      "Calls another workflow as a reusable step. Passes data in, waits for completion, and receives the sub-workflow's output.",
    fields: [
      {
        name: "Workflow",
        desc: "The target workflow to invoke (selected by name or ID).",
      },
      {
        name: "Input",
        desc: "Data to pass as the sub-workflow's trigger payload.",
      },
      {
        name: "Wait for completion",
        desc: "Block until the sub-workflow finishes (default). Disable for fire-and-forget.",
      },
    ],
  ...
  slack: {
    summary:
      "Sends a message or file to a Slack channel or DM using a configured Slack integration.",
    fields: [
      { name: "Channel", desc: "Target channel name or user ID." },
      {
        name: "Message",
        desc: "Text body. Supports Slack markdown and {{variable}} interpolation.",
      },
      {
        name: "Blocks",
        desc: "Optional Slack Block Kit JSON for rich message formatting.",
      },
      {
        name: "Thread TS",
        desc: "If set, posts as a reply in an existing thread.",
      },
    ],
  ...
  github: {
    summary:
      "Interacts with GitHub via the GitHub API — create issues, comment on PRs, push files, and more.",
    fields: [
      {
        name: "Action",
        desc: "Operation to perform: Create Issue, Add Comment, Create PR, Push File, etc.",
      },
      { name: "Repository", desc: "Owner/repo string (e.g. acme/backend)." },
      {
        name: "Payload",
        desc: "Action-specific fields (title, body, branch, path, etc.). Supports {{variable}} interpolation.",
      },
    ],
  ...
  notion: {
    summary:
      "Reads from or writes to Notion databases and pages using a configured Notion integration.",
    fields: [
      {
        name: "Action",
        desc: "Create Page, Update Page, Query Database, Append Block, or Read Page.",
      },
  ...
  gmail: {
    summary:
      "Sends emails or reads messages via Gmail using a configured Google account.",
    fields: [
      {
        name: "Action",
        desc: "Send Email, Read Emails, Search Emails, or Reply.",
      },
      { name: "To / From", desc: "Recipient or sender address filter." },
      {
        name: "Subject",
        desc: "Email subject line. Supports {{variable}} interpolation.",
      },
      { name: "Body", desc: "Plain text or HTML email body." },
      {
        name: "Query",
        desc: 'Gmail search query for Read/Search actions (e.g. "from:boss@acme.com is:unread").',
      },
    ],
  ...
  filter: {
    summary:
      "Filters an array to keep only elements matching a condition. Uses Jexl expressions evaluated per item.",
    fields: [
      {
        name: "Source",
        desc: "Variable name holding the array to filter. Defaults to lastOutput.",
      },
      {
        name: "Condition",
        desc: "Jexl expression evaluated for each element. Use `item` for the element and `index` for its position.",
      },
    ],
    inputs: ["array — the source array from an upstream node"],
    outputs: [
      "filtered — array containing only elements where the condition is truthy",
    ],
    tips: [
  ...
  merge: {
    summary:
      "Combines multiple arrays or objects into one. Supports concat, deep-merge, and zip modes.",
    fields: [
      {
        name: "Mode",
        desc: "Concat — flatten arrays into one; Merge — deep-merge objects; Zip — pair elements by index.",
      },
      {
        name: "Sources",
        desc: "Variable names (one per field) holding the arrays or objects to combine.",
      },
    ],
  ...
  datetime: {
    summary:
      "Formats, parses, and manipulates dates and times without external dependencies.",
    fields: [
      {
        name: "Operation",
        desc: "Now, Format, Parse, Add, Subtract, or Diff.",
      },
      {
        name: "Input",
        desc: "Variable name holding the date string, ISO timestamp, or Unix epoch to operate on.",
      },
      {
        name: "Amount / Unit",
        desc: "For Add/Subtract: the quantity and time unit (days, hours, months, etc.).",
      },
      {
        name: "Format",
        desc: "Output template. Supports YYYY, MM, DD, HH, mm, ss, ISO, UNIX, UTC.",
      },
    ],
    inputs: [
      "date — string, ISO timestamp, or Unix epoch from an upstream node",
    ],
    outputs: [
      "result — formatted string, parsed components object, or duration object",
    ],
    tips: [
  ...
  note: {
    summary:
      "A non-executing canvas annotation. Use notes to document your workflow with context, reminders, or section labels.",
    tips: [
  ...
  frame: {
    summary:
      "A resizable container for visually grouping related nodes. Frames have no effect on execution — they are purely organizational.",
    fields: [
      {
        name: "Label",
        desc: "Double-click the frame name to rename it inline.",
      },
      {
        name: "Color",
        desc: "Pick a border color from the palette shown when the frame is selected.",
      },
      {
        name: "Collapse",
        desc: "Click the arrow in the frame header to collapse it to just a title bar.",
      },
      {
        name: "Resize",
        desc: "Drag the corner handles (visible when selected) to resize the frame.",
      },
    ],
    tips: [
    ...
    return (
      ...
      >
        ...
        <p className="text-xs text-muted-foreground">
          No reference docs available for this node type.
        </p>
      </div>
    )
  }
...
export function NodePanel({
  ...
  function renderSubPanel() {
    switch (nodeType) {
      ...
      case "router":
        return <RouterPanel data={nodeData} onUpdate={handleUpdate} />
      case "start":
        return <StartPanel data={nodeData} onUpdate={handleUpdate} />
      case "approval":
        return <ApprovalPanel data={nodeData} onUpdate={handleUpdate} />
      case "mcp":
        return <McpPanel data={nodeData} onUpdate={handleUpdate} />
      case "memory":
        return <MemoryPanel data={nodeData} onUpdate={handleUpdate} />
      case "extract":
        return (
          <ExtractPanel
            data={nodeData}
      ...
      case "guardrails":
        return <GuardrailsPanel data={nodeData} onUpdate={handleUpdate} />
      case "code":
        return <CodePanel data={nodeData} onUpdate={handleUpdate} />
      case "loop":
        return (
          <LoopPanel
            data={nodeData}
      ...
      case "parallel":
        return <ParallelPanel data={nodeData} onUpdate={handleUpdate} />
      case "wait":
        return <WaitPanel data={nodeData} onUpdate={handleUpdate} />
      case "variables":
        return (
          <VariablesPanel
            data={nodeData}
      ...
      case "subworkflow":
        return <SubworkflowPanel data={nodeData} onUpdate={handleUpdate} />
      case "slack":
        return <SlackPanel data={nodeData} onUpdate={handleUpdate} />
      case "github":
        return <GitHubPanel data={nodeData} onUpdate={handleUpdate} />
      case "notion":
        return <NotionPanel data={nodeData} onUpdate={handleUpdate} />
      case "gmail":
        return <GmailPanel data={nodeData} onUpdate={handleUpdate} />
      case "filter":
        return <FilterPanel data={nodeData} onUpdate={handleUpdate} />
      case "merge":
        return <MergePanel data={nodeData} onUpdate={handleUpdate} />
      case "datetime":
        return <DatetimePanel data={nodeData} onUpdate={handleUpdate} />
      case "note":
        return (
          <p className="text-xs text-muted-foreground">
