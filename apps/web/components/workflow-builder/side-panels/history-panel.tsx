...
export function HistoryPanel({ workspaceId, podId, workflowId, nodes, onClose }: Props) {
  ...
  function durationLabel(exec: Execution): string {
    ...
    const ms = new Date(exec.finishedAt).getTime() - new Date(exec.createdAt).getTime();
            ...
            executions.map((ex) => (
              ...
              >
                <div className="flex items-center gap-1.5">
                  <span className={`size-1.5 shrink-0 rounded-full ${STATUS_DOT[ex.status] ?? 'bg-muted-foreground'}`} />
                  <span className={`text-[10px] font-semibold capitalize ${STATUS_COLOR[ex.status] ?? 'text-muted-foreground'}`}>
                    {ex.status}
                  </span>
                </div>
                <p className="mt-0.5 text-[10px] text-muted-foreground">{new Date(ex.createdAt).toLocaleString()}</p>
                <p className="text-[10px] text-muted-foreground">{durationLabel(ex)}</p>
              </button>
            ))
          )}
        </div>
        ...
        <ScrollArea className="flex-1">
          {!selectedId ? (
            <div className="flex flex-col items-center justify-center gap-2 py-12 text-center">
              <HugeiconsIcon icon={PlayIcon} className="size-5 text-muted-foreground/40" />
              <p className="text-xs text-muted-foreground">Select a run to see its logs</p>
            </div>
          ) : logsLoading ? (
              ...
              {logs.map((log) => (
                <div key={log.id} className="px-3 py-2 space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] font-semibold text-foreground">{getNodeName(log.nodeId)}</span>
                    <div className="flex items-center gap-2">
                      {log.durationMs != null && (
                        <span className="text-[10px] text-muted-foreground">{log.durationMs}ms</span>
                      )}
                      <span className={`text-[10px] font-semibold ${log.level === 'error' ? 'text-red-500' : 'text-green-600'}`}>
                        {log.level === 'error' ? 'failed' : 'ok'}
                      </span>
                    </div>
                  </div>
                  {(log.data?.output !== undefined || log.data?.error) && (
                    <pre className="text-[10px] font-mono whitespace-pre-wrap break-all text-muted-foreground max-h-24 overflow-y-auto bg-muted/30 rounded p-1.5">
                      {log.data?.error
                        ? log.data.error
