...
| `executions.ts` | `executions`, `execution_steps`, `execution_logs` |
...
## Two-Level Resource Hierarchy

```
workspaces
└── pods (workspaceId FK)
    ├── workflows (podId FK)
    ├── executions (podId FK + workspaceId — kept for quota lookups without JOIN)
    ├── schedules (podId FK)
    └── webhooks (podId FK)
```

Workspace-wide tables (no podId): `api_keys`, `secrets`, `linea_api_keys`, `mcp_servers`, `memories`, `knowledge_bases`, `resource_pools`.
...
// Active workflows
.where(and(eq(workflows.podId, podId), isNull(workflows.deletedAt)))

// Trashed workflows
.where(and(eq(workflows.podId, podId), isNotNull(workflows.deletedAt)))
```
