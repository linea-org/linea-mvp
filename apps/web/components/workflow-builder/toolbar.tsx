...
>>>>>>> bfb8587 (LIN-53: Codebase cleanup - split oversized files, fix AI-slop patterns, audit fixes (#117))
      ...
      <TooltipContent
        side="bottom"
        sideOffset={6}
        className="flex max-w-48 flex-col gap-0.5"
      >
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-muted-foreground">
                Press{" "}
                <kbd className="rounded border border-border bg-muted px-1 py-px text-[9px] font-medium">
                  Esc
                </kbd>{" "}
                or{" "}
                <kbd className="rounded border border-border bg-muted px-1 py-px text-[9px] font-medium">
        ...
        <div className="overflow-y-auto p-5">
          <div className="grid grid-cols-2 gap-x-8 gap-y-6">
            {SHORTCUT_GROUPS.map((group) => (
              <div key={group.label}>
                <p className="mb-3 text-[9px] font-bold tracking-widest text-muted-foreground uppercase">
                  {group.label}
                </p>
...
function ValidationBadge({ state }: { state: ValidationState }) {
  ...
  const cfg = {
    error: {
      ...
      popoverText: "text-red-600 dark:text-red-400",
      heading: "Errors",
      label: issues.length === 1 ? "1 error" : `${issues.length} errors`,
    },
    warning: {
      dot: "bg-amber-500",
      ring: "",
      text: "text-amber-600 dark:text-amber-400",
      popoverText: "text-amber-600 dark:text-amber-400",
      heading: "Warnings",
      label: issues.length === 1 ? "1 warning" : `${issues.length} warnings`,
    },
    success: {
      dot: "bg-green-500",
      ring: "",
  ...
  return (
    ...
    >
      <span
        className={`flex cursor-default items-center gap-1.5 text-xs font-medium ${cfg.text}`}
      >
        ...
        <div className="absolute top-full right-0 z-50 mt-2 w-60 rounded-lg border bg-popover p-2.5 shadow-lg">
          ...
          <ul className="space-y-1.5">
            {issues.map((issue, i) => (
              <li
                key={i}
                className={`flex items-start gap-1.5 text-xs ${cfg.popoverText}`}
              >
...
>>>>>>> bfb8587 (LIN-53: Codebase cleanup - split oversized files, fix AI-slop patterns, audit fixes (#117))
  ...
  return (
    <TooltipProvider delayDuration={500}>
      <div className="z-10 flex h-12 shrink-0 items-center justify-between border-b border-border bg-background px-3">
        <div className="flex items-center gap-2">
          <TBtn
...
>>>>>>> bfb8587 (LIN-53: Codebase cleanup - split oversized files, fix AI-slop patterns, audit fixes (#117))
        ...
        <div className="flex items-center gap-2">
          {runStatus && (
            <span
              className={`flex items-center gap-1.5 text-xs font-medium capitalize ${STATUS_COLOR[runStatus.status] ?? "text-muted-foreground"}`}
            >
...
>>>>>>> bfb8587 (LIN-53: Codebase cleanup - split oversized files, fix AI-slop patterns, audit fixes (#117))
          ...
          <Tooltip>
            ...
            >
              ...
              <span className="text-[10px] opacity-70">
                Describe a workflow in plain text
              </span>
            </TooltipContent>
          </Tooltip>
          ...
          <TBtn
            ...
            description={
              autoSave
                ? "Saves every 30s — click to disable"
                : "Click to enable auto-save every 30s"
            }
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              ...
              >
                <HugeiconsIcon
                  icon={isSaving ? Loading01Icon : FloppyDiskIcon}
          ...
          <Tooltip>
            ...
            >
              <span>
                {isDeployed ? "Deployment & triggers" : "Deploy workflow"}
              </span>
              <span className="text-[10px] opacity-70">
                Configure deployment, webhooks and REST API
              </span>
            </TooltipContent>
          </Tooltip>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                ...
                className={`flex size-7 items-center justify-center rounded-md border text-muted-foreground transition-colors hover:bg-muted/50 hover:text-foreground ${shortcutsOpen ? "border-border bg-muted text-foreground" : "border-border"}`}
              >
