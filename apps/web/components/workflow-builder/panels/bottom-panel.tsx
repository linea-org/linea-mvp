...
function StatusIcon({ status }: { status: string }) {
  ...
  return (
    <span className="inline-block size-3 shrink-0 rounded-full border border-border bg-muted/60" />
  )
}
...
function LogsTab({
  ...
  const logMap = useMemo<Map<string, Log>>(() => {
    ...
    for (const l of logs) {
      ...
      if (!ex || new Date(l.timestamp) >= new Date(ex.timestamp))
        m.set(l.nodeId, l)
    }
    return m
  }, [logs])
  ...
  function copyNode(row: NodeRow) {
    ...
    else if (row.output !== undefined)
      parts.push(
        `Output: ${typeof row.output === "string" ? row.output : JSON.stringify(row.output, null, 2)}`
      )
    ...
    const lines = rows
      ...
      .map((r) => {
        ...
        if (r.error) parts.push(`Error: ${r.error}`)
          ...
          parts.push(
            `Output: ${typeof r.output === "string" ? r.output : JSON.stringify(r.output, null, 2)}`
          )
        return parts.join("\n")
      })
      .join("\n\n")
    ...
    if (isDone) {
      return (
        <div className="flex h-full flex-col items-center justify-center gap-2 py-8 text-center">
          ...
          />
          <p className="text-xs text-muted-foreground">
            No node logs stored for this run.
          </p>
          <Link
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      {/* Toolbar */}
      <div className="flex shrink-0 items-center justify-between border-b border-border/30 bg-muted/20 px-3 py-1">
        <span className="text-[10px] text-muted-foreground">
          {rows.filter((r) => r.status !== "pending").length} / {rows.length}{" "}
          nodes
        </span>
        <div className="flex items-center gap-0.5">
          ...
          >
            <HugeiconsIcon
              icon={Copy01Icon}
              className={`size-3 ${copied ? "text-green-500" : ""}`}
            />
            {copied ? "Copied!" : "Copy"}
          </button>
          <button
      ...
      <div className="flex-1 overflow-auto">
        {runStatus?.status === "completed" && executionOutput !== undefined && (
          <div className="mx-3 mt-3 mb-1 rounded-lg border border-green-200 bg-green-50/60 px-3 py-2.5 dark:border-green-900 dark:bg-green-950/20">
            <p className="mb-1.5 text-[10px] font-semibold tracking-wider text-green-700 uppercase dark:text-green-400">
              Workflow Output
            </p>
            <pre className="max-h-28 overflow-y-auto font-mono text-[11px] leading-relaxed break-all whitespace-pre-wrap text-foreground">
        ...
        <table className="w-full border-collapse text-xs">
          <thead className="sticky top-0 z-10 bg-muted/60 backdrop-blur-sm">
            <tr>
              <th className="w-6 px-3 py-1.5 text-left text-[10px] font-semibold tracking-wider text-muted-foreground uppercase" />
              <th className="px-3 py-1.5 text-left text-[10px] font-semibold tracking-wider text-muted-foreground uppercase">
                Node
              </th>
              <th className="px-3 py-1.5 text-left text-[10px] font-semibold tracking-wider text-muted-foreground uppercase">
                Status
              ...
              <th className="w-16 px-3 py-1.5 text-left text-[10px] font-semibold tracking-wider text-muted-foreground uppercase" />
            </tr>
          </thead>
          <tbody>
            {rows.map((row) => {
                ...
                <Fragment key={row.id}>
                  <tr
                    ...
                    onClick={() =>
                      !isPending &&
                      hasDetail &&
                      setExpandedId(isExp ? null : row.id)
                    }
                  >
                    ...
                    <td
                      className={`px-3 py-2 font-medium capitalize ${isPending ? "text-muted-foreground" : statusColor(row.status)}`}
                    >
                      ...
                      <div className="flex items-center gap-0.5">
                        ...
                        {row.status === "failed" && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation()
                              void retryNode(row.id)
                            }}
                            disabled={retrying === row.id}
                            title="Retry this node"
                            className="flex items-center gap-1 rounded px-1.5 py-0.5 text-[10px] font-medium text-muted-foreground transition-colors hover:bg-muted hover:text-foreground disabled:opacity-50"
                          >
                            {retrying === row.id ? (
                              <HugeiconsIcon
                                icon={Loading01Icon}
                  ...
                  {isExp && hasDetail && (
                    <tr className="border-b border-border/40 bg-muted/10">
                      <td colSpan={6} className="px-3 py-2">
                        <pre className="max-h-36 overflow-y-auto font-mono text-[10px] leading-relaxed break-all whitespace-pre-wrap text-foreground">
                          {row.error
                            ? row.error
                  ...
                  {row.status === "running" && streamingTokens?.[row.id] && (
                    <tr className="border-b border-border/40 bg-muted/5">
                      <td colSpan={6} className="px-3 py-1.5">
                        <pre className="max-h-24 overflow-y-auto font-mono text-[10px] leading-relaxed break-all whitespace-pre-wrap text-muted-foreground">
  ...
  if (isDone && logs.length > 0) {
    ...
    entries = Object.entries(nodeResults)
      ...
      .map(([nodeId, r]) => ({
        ...
        durationMs:
          r.durationMs ??
          (r.status === "running" ? Date.now() - r.startedAt! : 0),
      }))
  }
      ...
      <div className="p-3">
        ...
        <div className="space-y-1.5">
          {entries.map((entry) => {
            ...
            return (
              <div key={entry.id} className="group flex items-center gap-2.5">
                ...
                <div className="relative h-5 flex-1 rounded bg-muted/50">
                  ...
                  {entry.status === "running" && (
                    <div
                      className="absolute top-0 h-full animate-pulse rounded"
                      style={{
                        left: `${leftPct}%`,
                        width: `${widthPct}%`,
                        backgroundColor: barColor,
        ...
        {/* Time axis ticks */}
        <div className="mt-2 mr-[64px] ml-[120px] flex">
          {[0, 0.25, 0.5, 0.75, 1].map((pct) => {
            ...
            return (
              <div
                key={pct}
                className="flex-1 font-mono text-[9px] text-muted-foreground/50"
                style={{
                  textAlign:
                    pct === 0 ? "left" : pct === 1 ? "right" : "center",
                }}
              >
                {label}
              </div>
  ...
  if (validationState.issues.length === 0) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-2">
        <HugeiconsIcon
          icon={CheckmarkCircle01Icon}
          className="size-6 text-green-500/60"
        />
        <p className="text-xs text-muted-foreground">
          No issues detected. Workflow looks good.
        </p>
      </div>
    )
  }
      ...
      <div className="space-y-1.5 p-3">
        {validationState.issues.map((issue, i) => (
          ...
          >
            <HugeiconsIcon
              icon={Alert02Icon}
              className={`mt-0.5 size-3.5 shrink-0 ${validationState.level === "error" ? "text-red-500" : "text-amber-500"}`}
            />
            <span
              className={
                validationState.level === "error"
...
function VariablesTab({
  ...
  if (startNode) {
    ...
    const testInput = startNode.data.testInput as
  ...
  for (const node of nodes) {
    if (node.type === "start" || node.type === "end" || node.type === "note")
      continue
  ...
  if (refs.length === 0) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-2">
        ...
        />
        <p className="text-xs text-muted-foreground">
          Add nodes to see available variable references.
        </p>
      </div>
    )
  }
    ...
    <ScrollArea className="h-full">
      <div className="space-y-1 p-3">
        {refs.map(({ path, value, source }) => (
          <div
            key={path}
            className="group flex items-start gap-2 rounded-lg px-2.5 py-2 transition-colors hover:bg-muted/40"
          >
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1.5">
                <code className="font-mono text-[11px] break-all text-violet-600 dark:text-violet-400">
                  {path}
                </code>
            ...
            <button
              onClick={() => copy(path)}
              title="Copy path"
              className="shrink-0 rounded p-1 text-muted-foreground opacity-0 transition-all group-hover:opacity-100 hover:text-foreground"
            >
              <HugeiconsIcon
                icon={Copy01Icon}
...
export function BottomPanel({
  ...
  const [autoOpenLogs, setAutoOpenLogs] = useState(() => {
    try {
      return localStorage.getItem("linea:logs:auto-open") !== "false"
    } catch {
      return true
    }
  })
  ...
  useEffect(() => {
    try {
      localStorage.setItem("linea:logs:auto-open", String(autoOpenLogs))
    } catch {
      /* ignore */
    }
  }, [autoOpenLogs])
  ...
  async function fetchHistory() {
    ...
    try {
      ...
      const list: HistoryEntry[] =
        (data as any)?.executions ?? (Array.isArray(data) ? data : [])
      setHistoryList(list)
    } catch {
      // silently fail
    }
  ...
  useEffect(() => {
    ...
    if (isDone && runStatus?.id !== prevRunId.current) {
      ...
      setHistoryList((prev) =>
        prev.map((e) =>
          e.id === runStatus!.id ? { ...e, status: runStatus!.status } : e
        )
      )
      void fetchHistory()
    }
  ...
  const effectiveRunStatus =
    runStatus ??
    (historyEntry ? { id: historyEntry.id, status: historyEntry.status } : null)
  ...
  return (
    <div className="shrink-0 border-t border-border bg-background select-none">
      <div className="flex h-8 items-center gap-0.5 px-2">
        {TABS.map((tab) => (
          ...
          >
            ...
            {tab.badge !== undefined && (
              <span
                className={`rounded-full px-1 py-0 text-[10px] leading-4 font-semibold ${
        ...
        <button
          ...
          title={
            autoOpenLogs
              ? "Logs auto-open on run: on"
              : "Logs auto-open on run: off"
          }
          className="mr-1 flex items-center gap-1 rounded px-2 py-0.5 text-[10px] text-muted-foreground transition-colors hover:bg-muted/50 hover:text-foreground"
        >
          <span
            className={`size-1.5 shrink-0 rounded-full ${autoOpenLogs ? "bg-blue-500" : "bg-muted-foreground/30"}`}
          />
          auto-open
        </button>
        ...
        {!isRunning && historyList.length > 0 && (
          <div ref={dropdownRef} className="relative mr-2">
            <button
              onClick={() => {
                setRunDropdownOpen((v) => !v)
                setRunSearch("")
              }}
              className="flex items-center gap-1.5 rounded px-2 py-0.5 text-[11px] transition-colors hover:bg-muted/50"
            >
              <StatusIcon status={effectiveRunStatus?.status ?? "pending"} />
              <span
                className={`font-medium capitalize ${statusColor(effectiveRunStatus?.status ?? "pending")}`}
              >
            ...
            {runDropdownOpen && (
              <div className="absolute right-0 bottom-full z-50 mb-1 w-64 rounded-lg border bg-popover shadow-lg">
                ...
                <div className="max-h-52 overflow-y-auto p-1">
                  {historyList
                    ...
                    .filter(({ entry }) => {
                      ...
                      return (
                        ...
                        formatRunTime(entry.createdAt).toLowerCase().includes(q)
                      )
                    })
                    .map(({ entry, idx }) => (
                      <button
                      ...
                      >
                        ...
                        <span
                          className={`shrink-0 font-medium capitalize ${statusColor(entry.status)}`}
                        >
                  ...
                  {historyList.length > 0 &&
                    runSearch &&
                    historyList.filter((e) => {
                      const q = runSearch.toLowerCase()
                      return (
                        e.status.includes(q) ||
                        formatRunTime(e.createdAt).toLowerCase().includes(q)
                      )
                    }).length === 0 && (
      ...
      {open && (
        <>
          <div
            className="h-1 cursor-row-resize bg-transparent transition-colors hover:bg-border/60"
