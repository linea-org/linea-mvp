...
export function NodePanel({ node, onClose, onUpdate, onDelete, nodes, edges, nodeResult, workspaceId, podId, executionId, onRetry }: NodePanelProps) {
  const getApi = useApiClient();
  const [activeTab, setActiveTab] = useState<PanelTab>('editor');
  const [editingName, setEditingName] = useState(false);
  const [localName, setLocalName] = useState(() => (node?.data.nodeName as string) ?? (node?.data.label as string) ?? '');
  const nameInputRef = useRef<HTMLInputElement>(null);
  const [debugResponse, setDebugResponse] = useState<string | null>(null);
  const [debugLoading, setDebugLoading] = useState(false);

  async function fetchDebugExplanation() {
    if (!workspaceId || !podId || !executionId || !node || !nodeResult?.error) return;
  ...
  function renderSubPanel() {
    switch (nodeType) {
      ...
      case 'if-else':  return <LogicPanel data={nodeData} onUpdate={handleUpdate} />;
