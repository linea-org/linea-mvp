...
```
Trigger (schedule / webhook / API)
  → ExecutionsService.createFromTrigger()
  → INSERT execution (status=pending)
  → BullMQ: enqueue job { executionId, workspaceId }
  → Worker picks up job
  → UPDATE status=running, startedAt=now()
  → Process steps
  → UPDATE status=completed|failed, completedAt=now(), output|error
```
