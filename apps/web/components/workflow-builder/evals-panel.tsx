...
function newCase(): TestCase {
  return {
    ...
    input: "{}",
    assertions: [
      { source: "output", path: "", operator: "exists", expected: "" },
    ],
    trials: 1,
  }
}
...
function getSourceCategory(source: string | undefined): SourceCategory {
  ...
  if (source === "tool_calls" || source.startsWith("tool_calls:"))
    return "tool_calls"
  return "metric"
}
...
export function EvalsPanel({
  ...
  function updateCase(id: string, patch: Partial<TestCase>) {
    onTestCasesChange(
      testCases.map((c) => (c.id === id ? { ...c, ...patch } : c))
    )
  }
  ...
  function updateAssertion(
    caseId: string,
    idx: number,
    patch: Partial<Assertion>
  ) {
    const tc = testCases.find((c) => c.id === caseId)
    if (!tc) return
    updateCase(caseId, {
      assertions: tc.assertions.map((a, i) =>
        i === idx ? { ...a, ...patch } : a
      ),
    })
  }
  ...
  function removeAssertion(caseId: string, idx: number) {
    ...
    if (!tc) return
    updateCase(caseId, {
      assertions: tc.assertions.filter((_, i) => i !== idx),
    })
  }
  ...
  async function runAll() {
    ...
    const payload = testCases.map((tc) => {
      ...
      return {
        ...
        input,
        trials: tc.trials ?? 1,
        scriptedResponses: tc.scriptedResponses?.filter(
          (r) => r.value.trim() !== "" || r.type !== "answer"
        ),
        assertions: tc.assertions.map((a) => {
          ...
          if (a.operator === "llm_judge") {
            return {
              ...
              threshold: a.threshold ? parseFloat(a.threshold) : 0.7,
            }
          }
          ...
          if (
            a.operator === "tool_called" ||
            a.operator === "tool_not_called"
          ) {
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      {results && (
        <div
          className={`shrink-0 border-b border-border px-4 py-2 text-xs font-medium ${passCount === totalCount ? "bg-green-50 text-green-700 dark:bg-green-950/30 dark:text-green-400" : "bg-red-50 text-red-700 dark:bg-red-950/30 dark:text-red-400"}`}
        >
            ...
            {testCases.map((tc) => {
              ...
              return (
                <div key={tc.id} className="border-b border-border">
                    ...
                    <div className="flex shrink-0 items-center gap-2">
                      {result?.trialResults && (
                        <span className="text-[10px] text-muted-foreground">
                          {result.trialResults.filter((t) => t.passed).length}/
                          {result.trialResults.length} trials
                        </span>
                      )}
                      <span className="text-[10px] text-muted-foreground">
                        {tc.assertions.length} assertion
                        {tc.assertions.length !== 1 ? "s" : ""}
                      </span>
                    </div>
                  </button>
                  ...
                  {/* Expanded content */}
                  {isExpanded && (
                    <div className="space-y-3 bg-muted/10 px-4 pt-1 pb-4">
                      {/* Name + delete */}
                      <div className="flex items-center gap-2">
                        <div className="flex-1 space-y-1">
                          ...
                          <Input
                            value={tc.name}
                            onChange={(e) =>
                              updateCase(tc.id, { name: e.target.value })
                            }
                        ...
                        <div className="shrink-0 space-y-1">
                          ...
                          <Select
                            ...
                            onValueChange={(v) =>
                              updateCase(tc.id, { trials: parseInt(v, 10) })
                            }
                          >
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-1">
                            ...
                            <span title="Pre-configured answers for approval/ask_human nodes. Consumed in order when the workflow suspends.">
                              <HugeiconsIcon
                                icon={Alert02Icon}
                                className="size-3 cursor-help text-muted-foreground/50"
                              />
                            </span>
                          </div>
                          <Button
                              ...
                              >
                                ...
                                <Select
                                  ...
                                  onValueChange={(v) => {
                                    ...
                                    updated[si] = {
                                      ...sr,
                                      type: v as ScriptedResponse["type"],
                                    }
                                    updateCase(tc.id, {
                                      scriptedResponses: updated,
                                    })
                                  }}
                                >
                        ...
                        {tc.assertions.length === 0 ? (
                          ...
                          <div className="space-y-2">
                            {tc.assertions.map((assertion, ai) => {
                              ...
                              const isToolOp =
                                op === "tool_called" || op === "tool_not_called"
                              const showPath =
                                (cat === "output" || cat === "node") &&
                                !isJudge &&
                                !isSemanticMatch &&
                              ...
                              return (
                                <div
                                  key={ai}
                                  className={`space-y-1.5 rounded-md border p-2 ${ar ? (ar.passed ? "border-green-300 bg-green-50/50 dark:border-green-800 dark:bg-green-950/20" : "border-red-300 bg-red-50/50 dark:border-red-800 dark:bg-red-950/20") : "border-border bg-background"}`}
                                >
                                  {/* Header row */}
                                  <div className="flex items-center gap-1.5">
                                    {ar && (
                                      <HugeiconsIcon
                                        icon={
                                          ar.passed ? Tick01Icon : Cancel01Icon
                                        }
                                    ...
                                    {ar?.score !== undefined && (
                                      <span
                                        className={`font-mono text-[10px] ${ar.passed ? "text-green-600" : "text-destructive"}`}
                                      >
                                  ...
                                  <div className="space-y-0.5">
                                    ...
                                    <Select
                                      ...
                                      onValueChange={(newSource) => {
                                        const newCat =
                                          getSourceCategory(newSource)
                                        const newOps =
                                          OPERATORS_FOR_SOURCE[newCat]
                                        const newOp = newOps.includes(op)
                                          ? op
                                          : newOps[0]!
                                        updateAssertion(tc.id, ai, {
                                          source: newSource,
                                          operator: newOp,
                                        })
                                      }}
                                    >
                                      ...
                                      <SelectContent>
                                        ...
                                        <SelectGroup>
                                          ...
                                          <SelectItem value="duration_ms">
                                            Duration (ms)
                                          </SelectItem>
                                          <SelectItem value="total_tokens">
                                            Total tokens
                                          </SelectItem>
                                          <SelectItem value="input_tokens">
                                            Input tokens
                                          </SelectItem>
                                          <SelectItem value="output_tokens">
                                            Output tokens
                                          </SelectItem>
                                        </SelectGroup>
                                        {agentNodes.length > 0 && (
                                          <SelectGroup>
                                            ...
                                            <SelectItem value="tool_calls">
                                              Any agent
                                            </SelectItem>
                                            {agentNodes.map((n) => (
                                              <SelectItem
                                                key={`tool_calls:${n.id}`}
                                                value={`tool_calls:${n.id}`}
                                              >
                                                {n.label}
                                              </SelectItem>
                                            ))}
                                  ...
                                  <div
                                    className={`grid gap-1.5 ${showPath ? "grid-cols-2" : "grid-cols-1"}`}
                                  >
                                    {showPath && (
                                      <div className="space-y-0.5">
                                        <label className="text-[9px] font-medium tracking-wide text-muted-foreground uppercase">
                                  ...
                                  {isJudge && (
                                    <div className="space-y-1.5">
                                      ...
                                      {ar?.reasoning && (
                                        <div
                                          className={`rounded px-2 py-1 text-[10px] ${ar.passed ? "bg-green-50/80 text-green-700 dark:bg-green-950/30 dark:text-green-400" : "bg-destructive/10 text-destructive"}`}
                                        >
                                          {ar.reasoning}
                                        </div>
                                      )}
                                  ...
                                  {ar &&
                                    ...
                                    !isSemanticMatch &&
                                    !isToolOp && (
                                      <div className="space-y-0.5 rounded bg-destructive/10 px-2 py-1 text-[10px] text-destructive">
                                        {ar.source &&
                                          ar.source !== "output" && (
                                            <div>
                                              source:{" "}
                      ...
                      {/* Trial breakdown */}
                      {result?.trialResults &&
                        result.trialResults.length > 1 && (
                          <div className="space-y-1">
                            <label className="text-[9px] font-medium tracking-wide text-muted-foreground uppercase">
                              Trial results ({Math.round(result.passRate * 100)}
                              % pass rate)
                            </label>
                            <div className="flex gap-1">
                              {result.trialResults.map((trial, ti) => (
                                <div
                                  ...
                                  className={`h-1.5 flex-1 rounded ${trial.passed ? "bg-green-500" : "bg-destructive/60"}`}
                                />
                              ))}
                            </div>
                          </div>
      ...
      {testCases.length > 0 && !running && (
        <div className="shrink-0 border-t border-border px-4 py-2">
          <p className="text-[10px] leading-snug text-muted-foreground">
            Eval cases are saved with the workflow. Save the workflow to persist
            them.
          </p>
        </div>
      )}
