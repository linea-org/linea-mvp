...
function extractInputVars(definition: unknown): string[] {
  ...
  const matches = [
    ...json.matchAll(/\{\{input\.([a-zA-Z_][a-zA-Z0-9_]*)\}\}/g),
  ];
...
export class PublicRunService {
  ...
  async trigger(
    ...
    if (row.apiEnabled) {
      if (row.apiVisibility === 'api_key') {
        if (!row.apiKey)
          throw new ForbiddenException(
            'This workflow has no API key configured',
          );
        ...
        if (
          expBuf.length !== prvBuf.length ||
          !timingSafeEqual(expBuf, prvBuf)
        ) {
          throw new UnauthorizedException('Invalid or missing API key');
        }
      }
      ...
      throw new ForbiddenException(
        'This workflow is not accessible via the public API',
      );
    }
  ...
  > {
    ...
    if (messageText) {
      const extracted = await this.extractVarsWithModel(
        messageText,
        missingVars,
        workspaceId,
        extractionModel,
...
- Return ONLY valid JSON with the extracted fields, nothing else`;
    ...
    return {
      ...
      output:
        row.status === 'completed'
          ? ((row.output as { result?: unknown } | null)?.result ?? null)
          : null,
      error: row.error,
  ...
  async setApiConfig(
    ...
    if (config.apiVisibility !== undefined)
      updates.apiVisibility = config.apiVisibility;
