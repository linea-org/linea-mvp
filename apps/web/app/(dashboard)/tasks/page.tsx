...
interface Attachment {
  id: string
  name: string
  type: "file" | "workflow" | "memory" | "connector"
  content?: string
  workflowId?: string
  fileType?: string
  connectorId?: string
...
const SLASH_COMMANDS = [
  {
    ...
    template: "Run the workflow ",
  },
  {
    cmd: "/create",
    icon: Add01Icon,
    description: "Create a new workflow",
    template: "Create a workflow that ",
  },
  {
    cmd: "/deploy",
    icon: CloudUploadIcon,
    description: "Deploy a workflow",
    template: "Deploy the workflow named ",
  },
  {
    cmd: "/schedule",
    icon: Calendar01Icon,
    description: "Schedule a workflow",
    template: "Schedule the workflow to run every ",
  },
  {
    cmd: "/search",
    icon: Search01Icon,
  ...
  },
  {
    ...
    template: "List all workflows in this pod",
  },
  {
    cmd: "/connect",
    icon: LinkSquare01Icon,
    description: "Check external connectors",
    template: "Show me my connected external integrations",
  },
  {
    cmd: "/status",
    icon: FlowIcon,
    description: "Check execution status",
    template: "What is the status of recent executions?",
  },
  {
    cmd: "/memory",
    icon: AiBrain01Icon,
    description: "Access memory & knowledge",
    template: "What do you know about ",
  },
  {
    cmd: "/debug",
    icon: Settings01Icon,
    description: "Debug a failed execution",
    template: "Why did my last execution fail?",
  },
  {
    cmd: "/summarize",
    icon: ArrowDown01Icon,
    description: "Summarize recent activity",
    template: "Summarize what happened in this pod today",
  },
  {
    cmd: "/help",
    icon: HelpCircleIcon,
...
const PRESETS = [
  {
    ...
    prompt:
      "Build a workflow that monitors my Gmail for important emails and posts them to a Slack channel",
  },
  {
    icon: ReloadIcon,
    label: "Daily sync & report",
    prompt:
      "Create a workflow that runs every morning to sync data and send me a summary report",
  },
  {
    icon: SourceCodeSquareIcon,
    label: "GitHub issue tracker",
    prompt:
      "Set up automated GitHub issue tracking with Slack notifications when issues are opened or assigned",
  },
  {
    icon: Search01Icon,
    label: "Check integrations",
    prompt:
      "Check what API keys and integrations I have configured in this workspace",
  },
  {
    icon: UserGroupIcon,
    label: "CRM automation",
    prompt:
      "Build a self-healing CRM that syncs and enriches customer data from my connected tools",
  },
  {
    icon: Calendar01Icon,
    label: "Morning briefing",
    prompt:
      "Create a workflow that sends me a daily morning briefing at 9am with key updates",
  },
]
...
function ThinkingSteps({ toolCalls }: { toolCalls: ToolCall[] }) {
  ...
  const activeName = !allDone
    ? (toolCalls.find((tc) => tc.result === undefined)?.name ?? "")
    : ""
  ...
  return (
    <div className="mb-2">
      <button
        onClick={() => setOpen((v) => !v)}
        className="group flex items-center gap-1.5 text-[11px] text-muted-foreground/70 transition-colors hover:text-muted-foreground"
      >
        {allDone ? (
          <span className="flex size-3 items-center justify-center rounded-full bg-muted text-[8px] text-muted-foreground">
            ✓
          </span>
        ) : (
          <HugeiconsIcon
      ...
      {open && (
        <div className="mt-1.5 space-y-1 border-l border-border/50 pl-4">
          {toolCalls.map((tc, i) => (
...
function ThinkingStepRow({ tc, index }: { tc: ToolCall; index: number }) {
  ...
  return (
    <div className="text-[11px]">
      ...
      >
        <span className="w-3.5 shrink-0 text-center text-[9px] text-muted-foreground/40">
          {index}.
        </span>
        {done ? (
          <span className="flex size-2.5 shrink-0 items-center justify-center rounded-full bg-muted-foreground/20 text-[7px] text-muted-foreground">
            ✓
          </span>
        ) : (
          <HugeiconsIcon
            icon={Loading01Icon}
            className="size-2.5 shrink-0 animate-spin text-muted-foreground/40"
          />
        )}
        <span className="italic">{TOOL_LABELS[tc.name] ?? tc.name}</span>
        {!done && (
          <span className="text-[9px] text-muted-foreground/40">running…</span>
        )}
        {hasDetails && (
      ...
      {open && hasDetails && (
        <div className="mt-1 mb-1 ml-5 space-y-1.5 rounded-md border border-border/40 bg-muted/10 px-2.5 py-2">
          {Object.keys(tc.input).length > 0 && (
            <div>
              <p className="mb-0.5 text-[9px] font-semibold tracking-widest text-muted-foreground/50 uppercase">
                Input
              </p>
              <pre className="font-mono text-[10px] break-all whitespace-pre-wrap text-muted-foreground/70">
    ...
    <div className="mt-3 overflow-hidden rounded-xl border border-primary/20 bg-primary/5">
      <div className="flex items-center gap-3 px-4 py-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 truncate text-sm font-semibold">{wf.name}</p>
        </div>
        {href && (
          <Link
            href={href}
            className="flex shrink-0 items-center gap-1.5 rounded-lg border border-primary/30 bg-background px-3 py-1.5 text-xs font-medium transition-colors hover:bg-muted"
          >
            Open
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith("```")) {
      ...
      elements.push(
        <pre
          key={i}
          className="my-2 overflow-x-auto rounded-lg border border-border bg-muted px-4 py-3 font-mono text-xs text-foreground"
        >
          {codeLines.join("\n")}
        </pre>
      )
    ...
    if (h1) {
      elements.push(
        <p key={i} className="mt-3 mb-1 text-base font-bold text-foreground">
          {h1[1]}
        </p>
      )
      i++
    ...
    if (h3) {
      elements.push(
        <p
          key={i}
          className="mt-2 mb-0.5 text-sm font-semibold text-foreground"
        >
          {h3[1]}
        </p>
      )
    ...
    if (bullet) {
      elements.push(
        <li
          key={i}
          className="ml-4 list-disc text-sm marker:text-muted-foreground"
        >
    ...
    if (num) {
      elements.push(
        <li
          key={i}
          className="ml-4 list-decimal text-sm marker:text-muted-foreground"
        >
          <InlineText text={num[1] ?? ""} />
        </li>
      )
      i++
      continue
...
function MessageBubble({
    ...
    return (
      <div className="flex justify-end">
        <div className="max-w-[80%] space-y-1.5">
          ...
          <div className="rounded-2xl rounded-tr-sm bg-muted px-4 py-2.5 text-sm leading-relaxed text-foreground">
            {msg.content}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="group flex gap-3">
      <div className="mt-1 flex size-7 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-violet-500 to-indigo-600 text-white shadow-sm">
                ...
                {onFeedback && (
                  <>
                    <button
                      ...
                      className={`rounded-md p-1.5 transition-colors ${feedbackVote === "up" ? "text-green-600" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}
                    >
                      <HugeiconsIcon icon={ThumbsUpIcon} className="size-3.5" />
                    </button>
                    <button
                      onClick={() => onFeedback("down")}
                      className={`rounded-md p-1.5 transition-colors ${feedbackVote === "down" ? "text-destructive" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}
                    >
                      <HugeiconsIcon
                        icon={ThumbsDownIcon}
                        className="size-3.5"
  ...
  return (
    <div className="flex h-full flex-col border-r border-border bg-muted/30">
      <div className="shrink-0 border-b border-border p-3">
        <button
          onClick={onNew}
          className="flex w-full items-center gap-2 rounded-lg border border-border bg-background px-3 py-2 text-xs font-medium text-foreground transition-colors hover:bg-muted"
        >
          <HugeiconsIcon
            icon={Add01Icon}
            ...
            {grouped.map((group) => (
              <div key={group.label}>
                ...
                {group.items.map((s) => (
                  <div
                    key={s.id}
                    className={`group relative mx-2 mb-0.5 flex items-center rounded-md ${s.id === currentId ? "bg-muted" : "hover:bg-muted/60"} transition-colors`}
                  >
                    <button
                      onClick={() => onLoad(s)}
                    ...
                    >
                      <p
                        className={`truncate text-xs font-medium ${s.id === currentId ? "text-foreground" : "text-muted-foreground group-hover:text-foreground"}`}
                      >
                        {s.title}
                      </p>
                    ...
                    <button
                      ...
                      className="mr-1 shrink-0 rounded p-1 text-muted-foreground opacity-0 transition-all group-hover:opacity-100 hover:bg-muted hover:text-foreground"
                    >
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          ...
          className="flex size-7 items-center justify-center rounded-full border border-border/60 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground disabled:opacity-40"
        >
        ...
        <DropdownMenuSub>
          ...
          <DropdownMenuSubContent className="w-44">
            {CONNECTOR_TYPES.map((c) => (
              <DropdownMenuItem
                key={c.id}
                onSelect={() => onConnector(c.id, c.label)}
              >
                <HugeiconsIcon
                  icon={c.icon}
                  className="size-3.5 shrink-0 text-muted-foreground"
...
function TasksPageInner() {
  ...
  async function upsertSession(
    ...
    try {
      ...
      setSessions((prev) =>
        prev.map((s) => (s.id === sid ? { ...s, dbId: saved.id } : s))
      )
    } catch {
      /* ignore */
    }
  ...
  async function deleteSession(id: string) {
    ...
    if (session?.dbId && activeWorkspace) {
      try {
        ...
        const api = createApiClient(token)
        await api.delete(
          `/workspaces/${activeWorkspace.id}/agent/sessions/${session.dbId}`
        )
      } catch {
        /* ignore */
      }
  ...
  async function send(overrideText?: string) {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE}/workspaces/${activeWorkspace.id}/agent/chat`,
        {
          method: "POST",
          headers: {
